...
def _get_keys() -> tuple[str | None, str | None]:
    try:
        return st.secrets["GOOGLE_API_KEY"], st.secrets["DEEPSEEK_API_KEY"]
    except Exception:
        return None, None
...
def page_processing():
    ...
    try:
        ...
        st.info("Pastikan DEEPSEEK_API_KEY valid dan akun DeepSeek memiliki kredit.")
...
def _sidebar():
    with st.sidebar:
        ...
        if ds_key:
            ...
            st.error("✗ DEEPSEEK_API_KEY tidak ditemukan")
        ...
        | Analisis teks | DeepSeek V3 |
        | Langkah kerja | DeepSeek V3 |
        | Edit konten | DeepSeek V3 |
        """)
