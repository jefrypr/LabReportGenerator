...
"""
Generator teks analisis, kesimpulan, dan langkah kerja menggunakan DeepSeek V3.
Mendukung streaming untuk UX yang lebih baik.
"""
...
# ─── Prompt Builder: Analisis & Kesimpulan ──────────────────────────────────

def _build_analysis_prompt(data: dict, seed: int) -> str:
    """
    Bangun prompt yang efisien untuk generate analisis dan kesimpulan.
    Seed memastikan variasi frasa tiap kali generate.
    """
    modul_snippet = ""
    ...
    # Format daftar pustaka dengan nomor
    refs_raw = data.get("daftar_pustaka", "").strip().split("\n")
    refs_formatted = []
    for i, ref in enumerate(refs_raw, 1):
        ref = ref.strip()
        if ref:
            # Hapus prefix nomor yang sudah ada jika pengguna sudah menulis "1."
            cleaned = ref.lstrip("0123456789.-) ").strip()
            refs_formatted.append(f"[{i}] {cleaned}")
    refs_text = "\n".join(refs_formatted) if refs_formatted else "tidak ada"

    prompt = f"""[ID-VARIASI: {seed}] — Penting: Gunakan frasa, struktur kalimat, dan urutan pembahasan yang BERBEDA dari versi sebelumnya. Hindari pengulangan pola kalimat.
...
# ─── Prompt Builder: Adjust/Edit Content ────────────────────────────────────
...
def generate_analysis(data: dict, api_key: str) -> Generator[str, None, None]:
    """
    Generate analisis dan kesimpulan dengan DeepSeek V3 (streaming).
    Seed acak memastikan frasa selalu berbeda tiap generate.
    """
...
def generate_langkah_kerja(context: dict, api_key: str) -> Generator[str, None, None]:
    """Generate langkah kerja praktikum (streaming)."""
    client = _get_client(api_key)
...
def adjust_content(
    ...
    """Edit/sesuaikan konten yang sudah ada berdasarkan instruksi pengguna (streaming)."""
    client = _get_client(api_key)
